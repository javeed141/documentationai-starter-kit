...
You can do similar queries on any of the fields in the [product schema](https://java.documentationai.com/docs/product-data-schema).  Here's an example that queries the `name` field:
...
```text
POST https://api.documentationai.com/v4/products/paginate?page=1&limit=50
{
  "query": "keys:*"
}
```
