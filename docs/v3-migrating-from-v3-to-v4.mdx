...
As a reminder, views specify which fields are returned in your search or download request and how those fields are formatted.  For example, the [products_basicFields](https://developer.datafiniti.co/v3/docs/products_basicfields) view for product data only returns a subset of available product fields, while the [products_pricesFlat](https://developer.datafiniti.co/v3/docs/products_pricesflat) view will split multiple prices for a single product into different rows when requesting CSV data.
