...
This will download every matching record in a CSV format.  We can also download it in JSON if we want.  You should also note that while the example records above show a truncated form, the actual records you download will have all the fields available in our [product schema](https://developer.documentationai.com/docs/product-data-schema).
