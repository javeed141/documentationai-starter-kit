...
```curl
curl --user AAAXXXXXXXXXXXX: 'https://api.documentationai.com/v3/data/products?view=products_all&format=JSON&q=categories:shoes&records=1&download=false'
```
...
| `api.documentationai.com` | The location of the API. |
...
Each product record will have multiple fields associated with it.  You can see a full list of available fields in our [Product Data Schema](https://java.documentationai.com/v3/docs/product-data-schema).
...
`https://AAAXXXXXXXXXXXX:@api.documentationai.com/v3/data/products?view=products_all&format=JSON&q=categories:shoes AND -categories:shirts AND categories:women AND (brand:* OR manufacturer:*) AND prices:*&records=10&download=false`
...
```curl
curl --user AAAXXXXXXXXXXXX: 'https://api.documentationai.com/v3/data/products?view=products_all&format=JSON&q=categories:shoes+AND+-categories:shirts+AND+categories:women+AND+%28brand%3A%2A+OR+manufacturer%3A%2A%29+AND+prices%3A%2A&records=10&download=false'
```
...
```curl
curl --user AAAXXXXXXXXXXXX: 'https://api.documentationai.com/v3/data/products?view=products_all&format=JSON&q=categories:shoes+AND+-categories:shirts+AND+categories:women+AND+%28brand%3A%2A+OR+manufacturer%3A%2A%29+AND+prices%3A%2A&download=true'
```
...
`https://AAAXXXXXXXXXXXX:@api.documentationai.com/v3/requests/XXXX`

```curl
curl --user AAAXXXXXXXXXXXX: 'https://api.documentationai.com/v3/requests/XXXX'
```
...
```curl
curl --user AAAXXXXXXXXXXXX: 'https://api.documentationai.com/v3/results/XXXX'
```
...
From here, you'll most likely want to process the data using the programming language of your choice. Our other [JSON guides](https://java.documentationai.com/v3/docs/getting-started-with-product-data) provide instructions on how to process this data.
