...
var APICall = 'https://' + APIToken + ':@api.documentationai.com/v3/data/properties?'
				+ 'view=' + view
				+ '&q=' + query
				+ '&format=' + format
				+ '&records=' + records
...
function checkDownloadUntilComplete(options, callback) {
	var downloadRequestAPICall = 'https://' + APIToken + ':@api.documentationai.com/v3/requests/' + options.requestID;
...
request({url : APICall}, function (error, response, body) {
  	...
  	checkDownloadUntilComplete ({requestID : requestID}, function (error, response) {
  		var resultsAPICall = 'https://' + APIToken + ':@api.documentationai.com/v3/results/' + requestID;
...
Since we've handled multiple steps of the download process in this code, we won't go into the details here, but we do recommend you familiarize yourself with those steps.  Checking them out in our [Property Data with Web Browser and CSV](https://java.documentationai.com/v3/docs/property-data-with-web-browser-and-csv) guide.
...
<Image src="https://files.readme.io/11f13d1-property_sample.jpg" width="1920" height="1080" alt="property_sample.jpg" />
