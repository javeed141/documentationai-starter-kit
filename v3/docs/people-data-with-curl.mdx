...
```curl
curl --request POST --url https://api.documentationai.com/v4/people/search --header 'authorization: Bearer AAAXXXXXXXXXXXX' --data '{"query":"jobTitle:CEO","num_records":1}'
```
...
| `"total_cost"` | The number of credits this request has cost you.  People records have a record cost multiplier tied to them.  [See pricing](https://documentationai.com/pricing/people-data-pricing/). |
...
Each product record will have multiple fields associated with it.  You can see a full list of available fields in our [People Data Schema](https://java.documentationai.com/docs/people-data-schema).
...
```curl
curl --request POST --url https://api.documentationai.com/v4/businesses/search --header 'authorization: Bearer AAAXXXXXXXXXXXX' --data '{"query":"jobTitle:CEO AND province:TX AND city:Austin", "num_records": 10}'
```
...
```curl
curl --request POST --url https://api.documentationai.com/v4/businesses/search --header 'authorization: Bearer AAAXXXXXXXXXXXX' --data '{"query":"jobTitle:\"Product Developer\"", "num_records": 10}'
```
...
```curl
curl --request POST --url https://api.documentationai.com/v4/businesses/search --header 'authorization: Bearer AAAXXXXXXXXXXXX' --data '{"query":"jobTitle:CEO AND province:TX AND city:Austin", "num_records": 20,"format":"csv","download": true}'
```
...
| `total_cost`     | The total number of credits used by this download.  People records have a record cost multiplier tied to them.  [See pricing](https://documentationai.com/pricing/people-data-pricing/).                   |
