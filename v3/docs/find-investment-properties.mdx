...
You can use the Datafiniti Property API to search for all kinds of properties that may match your investment criteria.  We have [hundreds of fields available in the data](https://developer.documentationai.com/docs/property-data-schema), all of which are searchable.  It's just a matter of finding out the right combination of filters to meet your needs.  Let's explore some common search criteria!
...
This query adds the `city` and `province` fields to provide some geo-targeting.  You can also run [radial searches around a lat/long point](https://developer.documentationai.com/docs/constructing-property-queries#geo-queries).
...
This query searches for single family homes priced between $300,000 and $500,000.  [Learn more about range queries](https://developer.documentationai.com/docs/constructing-property-queries#range-queries).
...
You can also export / view your data via our residential view. The residential view will display select available fields in our [Property Data Schema](https://java.documentationai.com/docs/property-data-schema) that is most relevant to residential `propertyType`. These `PropertyType`s are:
...
This will show us all multi-family homes that have rental units available.  [Here are some other possible values for this field](https://developer.documentationai.com/docs/possible-values-for-property-fields#propertytype).
...
This will search for land that's more than 1 acre in size.  [More info on searching on lot size](https://developer.documentationai.com/docs/possible-values-for-property-fields#floorsizeunit--lotsizeunit).
